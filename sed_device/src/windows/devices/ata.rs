...
impl Device for AtaDevice {
    ...
    async fn security_send(
        ...
        Ok(self
            .ioctl_device
            .trusted_send(security_protocol, protocol_specific, aligned_data.as_padded_slice())
            .await?)
    }
    ...
    async fn security_recv(
        ...
        let protocol_specific = u16::from_be_bytes(protocol_specific);
        self.ioctl_device
            .trusted_receive(security_protocol, protocol_specific, data.as_padded_mut_slice())
            .await?;
        Ok(data.into_vec())
    }
}

/// See [`super::super::scsi`] for info about alignment.
...
mod ioctl {
    ...
    impl AtaIoctlDevice for IoctlDevice {
        async fn identify_device(&self) -> Result<IdentifyDevice, DeviceError> {
            ...
            let _ = self.ioctl_symmetric(IOCTL_ATA_PASS_THROUGH_DIRECT, request_buffer.as_mut_slice()).await?;
